...
namespace NetInfo.Audit.Cisco.IOS.Router {
  ...
  /// <summary>
  /// Ensure this command exists and uses the correct source IP address: "snmp-server source-interface informs vlanXX" (see comment)
  ///
  ...
  /// </summary>
  public class IR169 : ISTIGItem {

    public IDevice Device { get; private set; }

    private static readonly string[] ApprovedSources = { "Vlan23", "Vlan51", "Vlan56" };

    public IR169(INMCIIOSDevice device) {
      this.Device = device;
    }

    public bool Compliant() {
      var device = (INMCIIOSDevice)Device;
      return ApprovedSources.Contains(device.SNMPSettings.SourceInterface);
    }
  }
}