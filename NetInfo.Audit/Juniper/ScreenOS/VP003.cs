...
namespace NetInfo.Audit.Juniper.ScreenOS {
  ...
  /// <summary>
  /// Validate the user "NRFK-ADMIN" password is current and the privilege is set to "all"
  /// </summary>
  public class VP003 : ISTIGItem {

    public IDevice Device { get; private set; }

    private readonly IEnumerable<string> validNrfkPasswords;

    public VP003(INMCIScreenOSDevice device, IEnumerable<string> validNrfkPasswords) {
      this.Device = device;
    ...
    public bool Compliant() {
      ...
      var pass = device.AdminSettings.Users
        .SingleOrDefault(c => c.Name.Equals("NRFK-ADMIN", StringComparison.OrdinalIgnoreCase));
