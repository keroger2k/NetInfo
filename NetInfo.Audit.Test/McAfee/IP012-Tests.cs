...
namespace NetInfo.Audit.Tests.McAfee {
  ...
  [TestFixture]
  public class IP012_Tests {
    private AssetBlob blob;

    [Test]
    public void IP012_should_return_true_when_correct_install_port_is_found() {
      blob = new AssetBlob {
...
Logging TCP Port : 8503"
      ...
      ISTIGItem item = new IP012(device);
    ...
    [Test]
    public void IP012_should_return_false_when_incorrect_install_port_is_found() {
      blob = new AssetBlob {
        Body = @"[Sensor Network Config]
IP Address : 10.24.192.110
Netmask : 255.255.255.224
