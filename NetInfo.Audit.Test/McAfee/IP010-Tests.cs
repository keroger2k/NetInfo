...
namespace NetInfo.Audit.Tests.McAfee {
  ...
  [TestFixture]
  public class IP010_Tests {
    private AssetBlob blob;

    [Test]
    public void IP010_should_return_true_when_correct_logging_port_is_found() {
      blob = new AssetBlob {
...
Logging TCP Port : 8503"
      ...
      ISTIGItem item = new IP010(device);
    ...
    [Test]
    public void IP010_should_return_false_when_incorrect_logging_port_is_found() {
      blob = new AssetBlob {
        Body = @"[Sensor Network Config]
IP Address : 10.24.192.110
Netmask : 255.255.255.224
